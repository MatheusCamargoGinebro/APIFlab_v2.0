...
async function change_user_admin_level(request, response) {
	...
	if (
		parseInt(targetUserLab.data.user_access_level) ===
		parseInt(user_admin_level)
	) {
		return response.status(400).json({
			status: false,
	...
	const result = await lab_models.updateUserLabRole(
		lab_id,
		user_id,
		user_admin_level
	);
...
async function add_user_to_lab(request, response) {
	...
	const result = await lab_models.addUserToLab(
		lab_id,
		user_admin_level,
		user_id
	);
